...
public:
  ...
  FactorKey addRelativePosePrior(
    ValueKey key1, ValueKey key2, const PoseWithNoise & deltaPose) override;
  FactorKey addAbsolutePosePrior(
    ValueKey key, const PoseWithNoise & pose) override;
  std::vector<FactorKey> addTagProjectionFactor(
    const Eigen::Matrix<double, 4, 2> & imgCorners,
    const Eigen::Matrix<double, 4, 3> & objCorners, const string & cameraName,
    const CameraIntrinsics & ci, double pixelNoise, ValueKey T_c_r,
    ValueKey T_r_w, ValueKey T_w_b, ValueKey T_b_o) override;
...
private:
  ...
  typedef std::unordered_map<double, gtsam::noiseModel::Isotropic::shared_ptr>
    PixelNoiseMap;
