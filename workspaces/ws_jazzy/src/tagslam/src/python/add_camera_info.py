...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='add camerainfo to bag.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument(
        '--out_bag', '-o', action='store', default='out.bag', help='name of the sliced output bag.'
    )
    parser.add_argument(
        '--start',
        '-s',
    ...
    parser.add_argument(
        '--image_topic', '-i', action='store', default=None, required=True, help='image topic.'
    )
    parser.add_argument(
        '--chunk_threshold',
        '-c',
    ...
    iterator = in_bag.read_messages(
        start_time=rospy.Time(args.start), end_time=rospy.Time(args.end)
    )
