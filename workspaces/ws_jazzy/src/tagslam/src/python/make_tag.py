...
#
#
# based on code by Thomas Schneider (Kalibr), Sept 2013
...
import numpy as np
from pyx import canvas, color, deco, path, style, unit
...
# borderBits must be consitent with the variable "blackBorder"
# in the detector code in file ethz_apriltag2/src/TagFamily.cc
...
def generateAprilTag(
    ...
    try:
        ...
        print(
            '[ERROR]: Requested tag ID of {0} not available in the {1} TagFamiliy'.format(
                tagID, tagFamililyData.chosenTagFamiliy
            )
        )
    ...
    c.fill(path.rect(xPos, yPos, metricSize, borderSize), [ccolor])  # bottom
    c.fill(
        path.rect(xPos, yPos + metricSize - borderSize, metricSize, borderSize), [ccolor]
    )  # top
    c.fill(
        path.rect(xPos + metricSize - borderSize, yPos, borderSize, metricSize), [ccolor]
    )  # left
    c.fill(path.rect(xPos, yPos, borderSize, metricSize), [ccolor])  # right
    ...
    # (decreases the effect of motion blur in the subpix refinement...)
    if symmCorners:
...
def generateAprilBoard(
    ...
    for y in range(0, n_rows):
        for x in range(0, n_cols):
            ...
            y_off = x_off
            pos = (
                margx * 100 + x * (1 + tagSpacing) * tagSize + x_off,
    ...
    if drawLegend:
        ...
        c.stroke(
            path.line(pos[0], pos[1], pos[0] + tagSize * 0.3, pos[1]),
            [
                color.rgb.red,
                deco.earrow(
                    [
                        deco.stroked([color.rgb.red, style.linejoin.round]),
        ...
        c.stroke(
            path.line(pos[0], pos[1], pos[0], pos[1] + tagSize * 0.3),
            [
                color.rgb.green,
                deco.earrow(
                    [
                        deco.stroked([color.rgb.green, style.linejoin.round]),
        ...
        caption = '{0}x{1} tags, size={2}cm and spacing={3}cm'.format(
            n_cols, n_rows, tagSize, tagSpacing * tagSize
        )
...
if __name__ == '__main__':
    ...
    genericOptions.add_argument(
        '--color', default='Black', dest='color', help='Color in cmyk space (default: %(default)s)'
    )
    ...
    aprilOptions.add_argument(
        ...
        dest='tagspacing',
        help=(
            'The space between the tags in fraction of the'
            + ' edge size [0..1] (default: %(default)s)'
        ),
    )
    aprilOptions.add_argument(
        '--tfam',
        default='t36h11',
        dest='tagfamiliy',
        help='Familiy of April tags {0} (default: %(default)s)'.format(
            AprilTagCodes.TagFamilies.keys()
        ),
    )
    aprilOptions.add_argument(
        '--startid',
    ...
    aprilOptions.add_argument(
        '--symm_corners', dest='symmCorners', action='store_true', help='add black corner squares'
    )
    aprilOptions.add_argument(
        '--no-symm_corners',
        dest='symmCorners',
    ...
    aprilOptions.add_argument(
        '--draw_box', dest='drawBox', action='store_true', help='draw thin black bounding box'
    )
    aprilOptions.add_argument(
        '--no-draw_box',
        dest='drawBox',
    ...
    generateAprilBoard(
        ...
        parsed.color,
        parsed.startid,
        borderBits=parsed.borderBits,
        symmCorners=parsed.symmCorners,
        drawBox=parsed.drawBox,
    )
