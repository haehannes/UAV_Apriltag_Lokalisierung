...
#include <tagslam/gtsam_optimizer.hpp>
...
namespace tagslam
{
static rclcpp::Logger get_logger() { return (rclcpp::get_logger("graph")); }

using boost::irange;
using std::string;

Graph::Graph() { optimizer_.reset(new GTSAMOptimizer()); }
...
double Graph::optimize(double thresh) { return (optimizer_->optimize(thresh)); }
...
VertexDesc Graph::insertVertex(const VertexPtr & vp)
{
...
VertexDesc Graph::insertFactor(const VertexPtr & vp)
{
...
std::vector<VertexDesc> Graph::getConnected(const VertexDesc & v) const
{
...
bool Graph::isOptimizableFactor(const VertexDesc & v) const
{
  if (graph_[v]->isValue()) {
...
std::vector<ValueKey> Graph::getOptKeysForFactor(
  VertexDesc fv, int numKeys) const
{
...
Graph::VertexToOptMap::const_iterator Graph::findOptimized(
  const VertexDesc & v) const
{
...
ValueKey Graph::findOptimizedPoseKey(const VertexDesc & v) const
{
  ...
  if (it->second.size() != 1) {
    BOMB_OUT(
      "pose must have one opt value: " << info(v)
                                       << " but has: " << it->second.size());
  }
  return (it->second[0]);
}

void Graph::markAsOptimized(
  const VertexDesc & v, const std::vector<FactorKey> & f)
{
...
VertexDesc Graph::addPose(uint64_t t, const string & name, bool isCameraPose)
{
...
bool Graph::hasPose(uint64_t t, const string & name) const
{
...
void Graph::print(const string & prefix) const
{
  for (auto v = boost::vertices(graph_); v.first != v.second; ++v.first) {
    ...
    LOG_DEBUG(
      prefix << " " << graph_[*v.first]->getLabel() << ":"
             << (isOpt ? "O" : "U"));
    if (graph_[*v.first]->isValue()) {
      PoseValueConstPtr vp =
        std::dynamic_pointer_cast<const value::Pose>(graph_[*v.first]);
    }
  }
}
...
double Graph::getError(const VertexDesc & v) const
{
  (void)v;  // XXX(Bernd) looks like a bug. Why not used??
...
std::vector<OptimizerKey> Graph::getOptimizerKeys(const VertexVec & vv) const
{
...
VertexVec Graph::getOptimizedFactors() const
{
  ...
  std::copy_if(
    ...
    [this](VertexDesc v) { return (optimized_.find(v) != optimized_.end()); });
  return (vv);
}

static double error_sum(
  const KeyToErrorMap & kem, const std::vector<OptimizerKey> & keys)
{
  double errSum(0);
...
void Graph::printErrorMap(const string & prefix) const
{
  ...
  for (const auto & v : errMap) {
    ...
    LOG_INFO(prefix << " " << FMT(8, 3) << v.first << " " << *vp);
  }
}
...
// static method!
string Graph::body_name(const string & body) { return ("body:" + body); }
// static method!
string Graph::cam_name(const string & cam) { return ("cam:" + cam); }
