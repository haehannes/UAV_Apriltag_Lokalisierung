...
//
static Transform rotate_to_z(const Eigen::Vector3d & translat, double * ang)
{
...
{
  Eigen::Matrix3d K;
  K << p(0), 2 * p(2), -p(0), p(1), 0.0, p(1), p(2), -2 * p(0), -p(2);
  return (K);
}
...
static Eigen::Matrix3d compute_FTF(
  const M33dVec & V_tilde, const Transform & R_z, const ObjPoints & p_tilde)
{
...
//
//
static void compute_polynomial(
  const Eigen::Matrix3d & FTF, double * f, double * g, double * h)
{
  ...
  double * beta_orig, double * beta_min, double * beta_max)
{
  ...
  *beta_orig = beta;
  ...
  const Transform R_z =
    (Transform)Eigen::AngleAxisd(gamma, Eigen::Vector3d::UnitZ());
  const Transform R_z0 =
    (Transform)Eigen::AngleAxisd(alpha, Eigen::Vector3d::UnitZ());
