...
{
  return (rclcpp::get_logger("graph_utils"));
}
...
public:
  ...
  void operator()(std::ostream & out, const VertexOrEdge & v) const
  {
    ...
    out << "[label=\"" << vp->getLabel() << "\", shape=" << vp->getShape()
        << ", color=" << color << "]";
  }
...
static AbsolutePosePriorFactorPtr find_abs_pose_prior(
  const Graph & g, const VertexDesc & vv)
{
  ...
  bool isCamPose)
{
  ...
  if (pwn.isValid()) {  // have valid pose prior
    ...
    const std::shared_ptr<factor::AbsolutePosePrior> fac(
      new factor::AbsolutePosePrior(t, pwn, name));
...
void add_body(Graph * g, const Body & body)
{
  ...
  LOG_INFO(
    "added body " << body.getName() << " with " << body.getTags().size()
                  << " tags");
}

void filter_graph(
  Graph * src, const std::set<VertexDesc> & factorsToRemove,
  ...
  double absPriorPositionNoise, double absPriorRotationNoise)
{
  ...
  for (const auto & srcf : srcfacs) {  // loop through factors
    LOG_DEBUG(" copying for factor " << src.info(srcf));
    for (const auto & srcv : src.getConnected(srcf)) {
      if (copiedVals.count(srcv) == 0) {  // avoid duplication
        ...
        if (pp) {
          ...
          PoseValuePtr srcpp = std::dynamic_pointer_cast<value::Pose>(srcvp);
          Transform pose = src.getOptimizedPose(srcv);
          const PoseNoise n =
            srcpp->isCameraPose()
              ? PoseNoise::make(0.05, 0.05)
              : PoseNoise::make(absPriorRotationNoise, absPriorPositionNoise);
          PoseWithNoise pwn(pose, n, true);
          AbsolutePosePriorFactorPtr pp(new factor::AbsolutePosePrior(
            destvp->getTime(), pwn, destvp->getName()));
...
void initialize_from(Graph * destg, const Graph & srcg)
{
  ...
  for (auto vi = srcg.getVertexIterator(); vi.first != vi.second; ++vi.first) {
    const VertexDesc sv = *vi.first;
    PoseValuePtr psp = std::dynamic_pointer_cast<value::Pose>(srcg[sv]);
    if (psp) {
      const VertexDesc dv = destg->find(psp->getId());
      if (!Graph::is_valid(dv)) {
        BOMB_OUT("cannot find dest value: " << psp->getLabel());
      }
      PoseValuePtr pdp = std::dynamic_pointer_cast<value::Pose>((*destg)[dv]);
      if (!pdp) {
...
{
  return (get_optimized_pose(g, 0, Graph::cam_name(cam.getName()), tf));
}
...
{
  return (get_optimized_pose(g, 0, Graph::tag_name(tag.getId()), tf));
}
