...
class ControlConsole(Node):
    def __init__(self):
        ...
        # ------------------------------------------------------------
        # Yaw-Rampe (Slew-Rate-Begrenzung)
        # ------------------------------------------------------------
        # Die Tasten q/e (und der Demo-/Track-Modus) setzen einen WUNSCH-Yaw
        # (self.target_yaw). Der TATSAECHLICH an PX4 gesendete Yaw
        # (self._yaw_sp) folgt diesem Wunsch nur mit begrenzter Winkel-
        # geschwindigkeit. Dadurch entsteht kein Yaw-Sprung mehr, den PX4
        # mit einem harten Drehmoment-Burst (bis MC_YAWRATE_MAX) aufholen
        # muesste. Ein solcher Burst laesst bei einem knapp motorisierten
        # Quad die Motoren saettigen und kostet Auftrieb -> Hoehenverlust.
        # Wert bewusst deutlich unter der PX4-internen Grenze halten.
        ...
        # Pause an jeder Ecke in Sekunden. Vor jedem 90-Grad-Richtungs-
        # wechsel haelt die Drohne kurz an, damit sie ausschwingen kann.
        # Ohne diese Pause springt der Geschwindigkeitsvektor des Setpoints
        # an der Ecke schlagartig um 90 Grad -> seitlicher Ruck. Mit Pause
        # bleibt die Bewegung durchgehend sanft.
        self.declare_parameter("demo_corner_pause_s", 1.0)
        # Gieren waehrend der Demo. False (Standard) = Yaw wird fuer die
        # gesamte Demo eingefroren, die Trajektorie wird OHNE Drehung
        # abgeflogen (sicherste Variante). Auf True nur setzen, wenn die
        # Drohne bewusst gieren soll.
        self.declare_parameter("demo_yaw_enabled", False)
        ...
        self.demo_corner_pause_s = float(
            self.get_parameter("demo_corner_pause_s").value
        )
        self.demo_yaw_enabled = bool(
            self.get_parameter("demo_yaw_enabled").value
        )
        ...
        self._demo_z = 0.0
        self._demo_yaw = 0.0
        # Zeitpunkt (monotonic), bis zu dem an einer Ecke pausiert wird.
        self._demo_pause_until = 0.0
        ...
        # Tatsaechlich gesendeter Yaw-Sollwert. Folgt self.target_yaw nur
        # mit begrenzter Winkelgeschwindigkeit (siehe _ramp_yaw). Startet
        # deckungsgleich mit dem Wunsch-Yaw, damit beim ersten Senden kein
        # Nachlauf entsteht.
        self._yaw_sp = self.target_yaw
    ...
    def _ramp_yaw(self, dt):
        """Fuehrt den gesendeten Yaw-Sollwert begrenzt an den Wunsch-Yaw heran.

        Bewegt ``self._yaw_sp`` pro Aufruf um hoechstens
        ``max_yaw_rate_rad_s * dt`` in Richtung ``self.target_yaw``, wobei die
        kuerzeste Drehrichtung ueber ``wrap_angle`` bestimmt wird (korrektes
        Verhalten am +/-pi-Uebergang). Dadurch dreht die Drohne unabhaengig
        davon, wie schnell q/e gedrueckt werden, nie schneller als die
        konfigurierte Winkelgeschwindigkeit.

        Args:
            dt: Zeitschritt seit dem letzten Aufruf in Sekunden.
        """
        err = wrap_angle(self.target_yaw - self._yaw_sp)
        max_step = self.max_yaw_rate_rad_s * dt
        if err > max_step:
            err = max_step
        elif err < -max_step:
            err = -max_step
        self._yaw_sp = wrap_angle(self._yaw_sp + err)
    ...
    def publish_setpoint(self):
        ...
        # --------------------------------------------------------------
        # Yaw-Rampe: den tatsaechlich gesendeten Yaw begrenzt nachfuehren.
        # Vor dem Arm (die Drohne steht/wird getrackt) kein Nachlauf ->
        # _yaw_sp direkt auf den Wunsch setzen, damit beim Armen kein
        # aufgestauter Yaw-Fehler abgedreht wird. Im Flug (armed) wird die
        # Slew-Rate-Begrenzung angewandt.
        # --------------------------------------------------------------
        dt = 1.0 / max(self.setpoint_publish_rate_hz, 0.1)
        if not armed_now:
            self._yaw_sp = self.target_yaw
        ...
        # Yaw kommt aus dem gerampten _yaw_sp (nicht direkt aus target_yaw),
        # damit PX4 nie einen Yaw-Sprung aufholen muss. yawspeed bleibt NaN:
        # da bereits der Positions-Sollwert langsam nachgefuehrt wird, folgt
        # PX4 ihm mit kleiner, unkritischer Gierrate.
        nx, ny, nz = enu_pos_to_ned(self.target_x, self.target_y, self.target_z)
        nyaw = yaw_enu_to_ned(self._yaw_sp)
    ...
    def start_demo(self):
        ...
        # Yaw wird auf den AKTUELL GESENDETEN Wert (_yaw_sp) eingefroren,
        # nicht auf einen evtl. noch nachlaufenden Wunsch-Yaw. So ist beim
        # Abfliegen der Trajektorie garantiert keine Restdrehung mehr aktiv.
        self._demo_z = self.target_z
        self._demo_yaw = self._yaw_sp
        # Eventuelle Eck-Pause zuruecksetzen.
        self._demo_pause_until = 0.0
    ...
    def _run_demo_step(self):
        ...
        Hoehe wird auf den eingefrorenen Wert gehalten. Der Yaw bleibt
        standardmaessig (demo_yaw_enabled=False) fuer die gesamte Demo
        eingefroren -- die Trajektorie wird also OHNE Drehung abgeflogen.
        X/Y werden pro Tick um speed * dt in Richtung des Wegpunkts
        verschoben. Ist der Wegpunkt erreicht, wird exakt auf ihn eingerastet
        und -- bevor es zum naechsten (um 90 Grad versetzten) Wegpunkt
        weitergeht -- kurz pausiert (demo_corner_pause_s), damit die Drohne
        ausschwingen kann und kein seitlicher Ruck entsteht. Nach dem letzten
        Wegpunkt (Startecke) endet die Demo, die Drohne schwebt dort.
        """
        # Hoehe waehrend der Demo konstant halten.
        self.target_z = self._demo_z
        # Yaw konstant halten, sofern nicht ausdruecklich freigegeben.
        if not self.demo_yaw_enabled:
        ...
        # Eck-Pause: an einer erreichten Ecke kurz halten, bevor die
        # Bewegungsrichtung wechselt. Waehrend der Pause bleibt der
        # Positions-Sollwert stehen (die Drohne haelt und schwingt aus).
        now = time.monotonic()
        if now < self._demo_pause_until:
            return
        ...
        if dist <= max(step, 1e-6):
            ...
            if self._demo_index >= len(self._demo_waypoints):
                ...
                # Vor dem naechsten Schenkel kurz ausschwingen lassen.
                self._demo_pause_until = now + self.demo_corner_pause_s
        else:
    ...
    def get_display_lines(self):
        ...
        lines.append(
            f"yaw (soll/ist): {math.degrees(self.target_yaw):.1f} / "
            f"{math.degrees(self._yaw_sp):.1f} deg  "
            f"(max {math.degrees(self.max_yaw_rate_rad_s):.0f} deg/s)"
        )
        ...
        lines.append(
            f"q/e: yaw -/+ (gerampt, max "
            f"{math.degrees(self.max_yaw_rate_rad_s):.0f} deg/s)"
        )
