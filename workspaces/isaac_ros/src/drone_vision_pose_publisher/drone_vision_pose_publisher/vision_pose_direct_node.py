...
def rodrigues_to_quat(rx, ry, rz):
    ...
    Dies ist die TagSLAM-Konvention fuer den 'rotation'-Block ohne 'w':
    Die Richtung des Vektors gibt die Drehachse an, seine Laenge den
    Drehwinkel in Radiant (entspricht dem Logarithmus der Rotationsmatrix,
    d.h. der Exponentialabbildung von so(3) nach SO(3)).

    Quelle: TagSLAM-Dokumentation (Concepts) -- Posen werden als
    Rotationsvektoren gespeichert, NICHT als Euler-/RPY-Winkel.

    Wichtig: Diese Darstellung ist fuer beliebige Tag-Orientierungen
    korrekt (auch Tags an Waenden oder schraeg im Raum), nicht nur fuer
    flach am Boden liegende Tags.

    Formel:
        theta = ||r||
        q = ( r/theta * sin(theta/2), cos(theta/2) )   fuer theta > 0
        q = (0, 0, 0, 1)                                 fuer theta -> 0
    """
    theta = math.sqrt(rx * rx + ry * ry + rz * rz)

    if theta < 1e-12:
        # Keine Drehung -> Identitaetsquaternion.
        return (0.0, 0.0, 0.0, 1.0)
...
def pose_to_transform(pose):
    ...
    Unterstuetzt zwei Rotations-Formate:
      - Quaternion: rotation-Block enthaelt 'w' (x, y, z, w).
      - Rodrigues / Achs-Winkel: rotation-Block ohne 'w' (x, y, z),
        interpretiert als Rotationsvektor (TagSLAM-Konvention).

    Die Felder 'position_noise' und 'rotation_noise' werden bewusst
    ignoriert -- sie sind nur fuer den TagSLAM-Optimierer relevant,
    nicht fuer die Live-Lokalisierung.
    """
    if pose is None:
        pose = {}
